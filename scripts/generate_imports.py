...
def generate_imports_bijectors(filename):
    ...
    with io.StringIO() as file:
        ...
        meta_str = ",\n    ".join(
            [f'("{b[0]}", {b[0]})' for b in meta_bijectors]
        )
        standard_str = ",\n    ".join(
...
if __name__ == "__main__":
    ...
    bijectors_init = os.path.join(
        flowtorch.__path__[0], "bijectors/__init__.py"
    )
    distributions_init = os.path.join(
        flowtorch.__path__[0], "distributions/__init__.py"
    )
    parameters_init = os.path.join(
        flowtorch.__path__[0], "parameters/__init__.py"
    )
