...
### Parent class
A bijector must inherit from [`flowtorch.Params`](https://github.com/facebookincubator/flowtorch/blob/master/flowtorch/param.py). This class defines important methods that are common to all parameter objects, such as `.__call__` for instantiating a [`flowtorch.ParamsModule`](https://github.com/facebookincubator/flowtorch/blob/master/flowtorch/param.py) given shape information.
...
:::info
Further metadata fields may be defined in the future. However, developers are not permitted to define their own without adding a default value to [`flowtorch.Params`](https://github.com/facebookincubator/flowtorch/blob/master/flowtorch/param.py).
:::
